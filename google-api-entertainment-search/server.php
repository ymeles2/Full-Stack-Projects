...
$request_type = isset($_GET["requestType"]) ? $_GET["requestType"] : null;
if ($request_type == "placeDetails") {
  get_place_details();
  exit; // avoid reloading the html page
}
else if ($request_type == "nearbyPlaces")  {
  get_nearby_places();
  exit;
}
...
function get_nearby_places() {
   ...
   $CONVERSION_FACTOR = 1609; // 1 mile in meters

   $keyword = str_replace(" ", "+", $_GET["keyword"]);
   $category = str_replace(" ", "+", $_GET["category"]);
   $distance = (int)str_replace(" ", "+", $_GET["distance"]) * $CONVERSION_FACTOR; 
...
function search() {
   ...
   if (qs != null) {
      ...
      xmlhttp.open("GET", "hw60ejRx29adf8Giv4n.php?"+qs, false);
      xmlhttp.send();
...
function getPlaceDetails(elemID) {
   ...
   var xmlhttp = new XMLHttpRequest();
   xmlhttp.open("GET", "hw60ejRx29adf8Giv4n.php?"+querystring, false);
   xmlhttp.send();
   jsonDoc = xmlhttp.responseText;
